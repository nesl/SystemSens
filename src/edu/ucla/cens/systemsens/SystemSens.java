...
import edu.ucla.cens.systemsens.util.PowerModel;
...
public class SystemSens extends Service
{
    ...
    private PowerModel mPowerModel;
    ...
    */
    ...
    public void onCreate() {
        ...
        mPowerModel = new PowerModel();

        Log.i(TAG, "About to read PowerDB");
        mPowerDB.readDeadline();
        mPowerModel.setDeadline(mPowerDB.getSetTime(),
                mPowerDB.getDeadline(),
                mPowerDB.getLevel());
        Status.setDeadline(mPowerDB.getDeadlineDate());

        mPolicy = mPowerModel.RATE_POLICY;
        ...
        new IPowerMonitor.Stub()
    {
        ...
        public void setDeadline(int deadline)
        {
            ...
            deadlineDate.add(Calendar.MINUTE, deadline);

            

            Status.setDeadline(deadlineDate);


            int curLevel = (int)Status.getLevel();
            Calendar curTime = Calendar.getInstance();
            mPowerModel.setDeadline(curTime, deadline, curLevel);

            mPowerDB.saveDeadline(curTime, deadline, curLevel);
    ...
    private RemoteCallbackList<IAdaptiveApplication> mClients =
        new RemoteCallbackList<IAdaptiveApplication>()
    {
        public void onCallbackDied(IAdaptiveApplication callback)
        {
        ...
        public void onReceive(Context context, Intent intent) 
        {
            ...
            if (Intent.ACTION_BATTERY_CHANGED.equals(action)) 
            {
                        ...
                        BatteryManager.BATTERY_STATUS_DISCHARGING) 
                {
                    ...
                    // I need to remember the voltage
                    mPowerModel.recordBatInfo(voltage, level, current);
                } 
                else if (status == 
    ...
    private void checkModel()
    {
        Thread modelThread = new Thread()
        {
            public void run()
            {
                ...
                Log.i(TAG, "Submitting the deadline");
                mPowerModel.submitDeadline();

                Log.i(TAG, "Retrieving the power model");
                mPowerModel.getModel();
            ...
            try
            {
                ...
                try
                {
                    for(int j = 0; j < workList.size(); j++)
                    {
                        ...
                        if (mPolicy.equals(mPowerModel.RATE_POLICY))
                        {
                            /* Rate-based policy */
                ...
                if (!bootStrap)
                {
                    app.setWorkLimit(nextWorkLimit);
                    Log.i(TAG, "Budget for " + clientName + ": " 
                            + nextWorkLimit.toString());
                }
